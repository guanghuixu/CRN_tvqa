...
@registry.register_loss("anls_reward")
class ANLS_REWARD(nn.Module):
    def __init__(self):
        super().__init__()
        self.one = torch.Tensor([1.])
        with open('./data/fixed_vocab_path.txt') as f:
            fixed_ans_path = f.readlines()
        fixed_ans_path = fixed_ans_path[0]
        print('We use {} to compute the anls_reward'.format(fixed_ans_path))
        self.anls_model = ANLSEvaluator(fixed_ans_path)
    ...
    def forward(self, sample_list, model_output):
        ...
        predicted_scores = scores.argmax(dim=-1)
        gt_answers = sample_list['gt_answers']
        ocr_tokens = sample_list.context_tokens
        batch_anls = self.anls_model.compute_anls(predicted_scores, gt_answers, ocr_tokens)
        scores, _ = F.softmax(scores, dim=-1).max(dim=-1)

        log_scores = torch.log(scores + 1e-12)
        log_scores *= loss_mask
        losses = -torch.Tensor(batch_anls).to(log_scores.device) * log_scores.sum(dim=-1)
